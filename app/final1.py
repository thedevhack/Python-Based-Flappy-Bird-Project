...
#game-window config and loading background img
...
#intializing different variables for scoring and the bird imag is loaded and
# the moving base and many other images of play and quit
# and here x denotes if we are playing(x=2) or paused the game(x=1)
...
#function for game start screen which will ask for play and quit
def start_screen(x):
    global pipe_x
    while x == 2:
        for event in pygame.event.get():
            ...
            if 45 < mouse[0] < 125 and 200 < mouse[1] < 300:
                if click[0] == 1:
                    pipe_x = window_width
                    game_loop()
                    x = 2
            if 150 < mouse[0] < 230 and 200 < mouse[1] < 300:
                if click[0] == 1:
                    pygame.quit()
                    quit()
        window.blit(bg, (0, 0))
        font_heading = pygame.font.SysFont("impact", 44, False, False)
        print_heading = font_heading.render("FLAPPY BIRD", True, orange)
        window.blit(print_heading, (40, 80))
        window.blit(play_button, (45, 200))
        window.blit(quit_button, (150, 200))
        pygame.display.update()
        Clock.tick()

#function which will be called when bird crashes and then it will show current score
# and highest score achieved
def crash_screen(x, count, high_score):
    global pipe_x
    while x == 1:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
...
#it will display and update the score of bird as it passes the pipe
...
#bird collision mechanism
        ...
        if bird_y < pipe_uh + 38:
            if bird_x + img_width == pipe_x or bird_x > pipe_x and bird_x < pipe_x + 52:
                crash_screen(1, count, high_score)
...
#the transform is used as it will change the length of pipes as
# pipes are images and not drawing so transform is used
...
#this is used to compare scores and update if current score is greater than previous high score
        if count > high_score:
            high_score = count
